...
let stripPos (decl:Module<'a*'b>) :Module<'a> = 
                     ...
                     foldExp {varF = (fun (s, l) -> Var s) 
                              ...
                              newF = (fun ss e -> Exp.New (LongIdent (List.map (fun (s,l) -> Ident s) ss), e))
                              ...
                              doF = (fun e -> Do e)
                              downcastF = (fun e t -> Downcast (e, LongIdent (List.map (fun (s,l) -> Ident s) t)))
                              upcastF = (fun e t -> Upcast (e, LongIdent (List.map (fun (s,l) -> Ident s) t)))
                              interfaceF = (fun t ms -> Interface (LongIdent (List.map (fun (s,l) -> Ident s) t), ms))
