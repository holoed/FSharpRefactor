      ...
      and LoopTypeInst t = 
                cont { match t with   
                       | LongIdent ts -> let! tsAcc = mmap LoopTypeInst ts
                                         return state { return! tLongIdentF tsAcc }                    
                       | Ident s -> return state { return! tIdentF s }
                       | Type.TVar v -> let! vAcc = LoopTypeInst v
                                        return state { return! tvarF vAcc }
                       | Type.TFun (t1, t2) -> let! t1Acc = LoopTypeInst t1
                                               let! t2Acc = LoopTypeInst t2
                                               return state { return! tfunF t1Acc t2Acc } }
      ...
      and LoopTypes t = 
               cont { match t with
                      ...
                      | Abbrev (name, ty) -> let! tyAcc = LoopTypeInst ty
                                             return state { return! (abbrevF name tyAcc ) } }
...
let foldExp  varF
     ...
     StateMonad.execute (foldExpState (fun x -> state { return varF x })
                                      ...
                                      (fun t ms -> state { let! tAcc = t
                                                           let! msAcc = mmapId ms
                                                           return interfaceF tAcc msAcc })
                                      (fun es -> state { let! esAcc = mmapId es
                                                         return letBindingsF esAcc })
                                      (fun n t -> state { let! tAcc = t
                                                          return abbrevF n tAcc })
                                      (fun t1 t2 -> state { let! t1Acc = t1
                                                            let! t2Acc = t2
                                                            return tfunF t1Acc t2Acc })
                                      (fun s -> state { return tIdentF s })
                                      (fun ts -> state { let! tsAcc = mmapId ts
