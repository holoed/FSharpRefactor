                          ...
                          while true do 
                              ...
                              let exitCode = 
                                  try 
                                      ...
                                      let createErrorLogger = (fun (tcConfigB:TcConfigBuilder) -> ErrorLoggerThatQuitsAfterMaxErrors(tcConfigB, exiter))
    ...
    let main argv = 
        ...
        if argv |> Array.exists  (fun x -> x = "/resident" || x = "--resident") then 
            ...
            if not (argv |> Array.exists (fun x -> x = "/nologo" || x = "--nologo")) then 
                printfn "%s" (FSComp.SR.buildProductName("Custom build"))
